...
public class LoginController {
    ...
    public void initialize() {
        ...
        // El logo y la tarjeta blanca crecen proporcional al tamaño de la ventana
        // (la ventana siempre abre maximizada, así que esto cubre pantallas grandes/chicas).
        if (rootLogin != null && imgLogo != null && loginCardInner != null) {
            rootLogin.sceneProperty().addListener((obs, escenaVieja, escenaNueva) -> {
                if (escenaNueva != null) {
 
                    imgLogo.fitWidthProperty().bind(
                            Bindings.createDoubleBinding(
                                    () -> clamp(escenaNueva.getWidth() * 0.11, 140, 260),
                                    escenaNueva.widthProperty()
                            )
                    );
                    imgLogo.fitHeightProperty().bind(imgLogo.fitWidthProperty());
 
                    loginCardInner.maxWidthProperty().bind(
                            Bindings.createDoubleBinding(
                                    () -> clamp(escenaNueva.getWidth() * 0.30, 420, 620),
                                    escenaNueva.widthProperty()
                            )
                    );
                    loginCardInner.minWidthProperty().bind(loginCardInner.maxWidthProperty());
                }
            });
        }
    ...
    private void onIrARegistro() {
        try {
            ...
            mostrarError("Error del sistema", "No fue posible abrir la pantalla de registro.");
            e.printStackTrace();
        }
    }
