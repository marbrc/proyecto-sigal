...
public class CuentaController implements Initializable {
    ...
    @FXML private Button btnDesactivarMiCuenta;
    ...
    @FXML
    private void onDesactivarMiCuenta(ActionEvent event) {
        if (usuarioActual == null) return;
 
        Alert confirmacion = new Alert(AlertType.CONFIRMATION,
                "Tu cuenta quedará desactivada temporalmente: no podrás iniciar sesión hasta que se reactive. "
                        + "¿Seguro que quieres continuar?");
        confirmacion.setTitle("Desactivar mi cuenta");
        confirmacion.setHeaderText(null);
        if (confirmacion.showAndWait().filter(b -> b == javafx.scene.control.ButtonType.OK).isEmpty()) {
            return;
        }
 
        boolean ok = usuarioDAO.desactivarUsuario(usuarioActual.getIdUsuario());
        if (!ok) {
            mostrarAlerta(AlertType.ERROR, "No se pudo desactivar",
                    "Ocurrió un problema al desactivar tu cuenta en la base de datos.");
            return;
        }
 
        mostrarAlerta(AlertType.INFORMATION, "Cuenta desactivada",
                "Tu cuenta se desactivó correctamente. Se cerrará tu sesión.");
